...
async def handle_comment_event(value: dict):
    """Logic for processing a single comment event."""
    ...
    ig_business_id = get_config("IG_BUSINESS_ID")
    ...
    rule = get_automation_for_comment(text, media_id)
    if not rule:
        return

    # 2. Duplicate Check (Skip for our test Admin account)
    ADMIN_TEST_ID = "1382909329391520"
    if user_id != ADMIN_TEST_ID and check_if_already_sent(user_id, media_id, rule["id"]):
        return
    ...
    success = send_private_reply(comment_id, dm_text)
    
    if success:
        print(f"✅ Private reply sent to @{username} for comment {comment_id}")
        mark_dm_as_sent(user_id, media_id, rule["id"])
    else:
        ...
        send_comment_reply(comment_id, public_msg)
        
        if rule["follow_gate"]:
            update_gate_status(user_id, media_id, 0)
